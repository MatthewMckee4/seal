...
pub trait Visit {
    /// Visits an [`Field`] value named `name`.
...
/// Metadata of an option that can either be a [`Field`] or [`OptionSet`].
...
struct SerializeVisitor<'a> {
    entries: &'a mut BTreeMap<String, Field>,
}
...
mod tests {
    ...
    fn test_find_child_option() {
        ...
        static IGNORE_GIT_IGNORE: Field = Field {
            doc: "Whether Ruff should respect the gitignore file",
            default: Some("false"),
            value_type: "bool",
            example: "",
