...
pub fn help(query: &[String], printer: Printer, no_pager: bool) -> Result<ExitStatus> {
    let mut seal: clap::Command = Cli::command();
    ...
    let command = find_command(query, &seal).map_err(|(unmatched, nearest)| {
        let missing = if unmatched.len() == query.len() {
            format!("`{}` for `seal`", query.join(" "))
        } else {
            format!(
                "`{}` for `seal {}`",
                unmatched.join(" "),
    ...
    let is_root = name == seal.get_name();
    ...
    let help = if is_root {
        ...
        if command.has_subcommands() {
            command.after_long_help(format!(
                "Use `{}` for more information on a specific command.",
                format!("seal help {name} <command>").bold()
            ))
        } else {
            command
        }
    ...
    if should_page && let Some(pager) = Pager::try_from_env() {
        ...
        if want_color && pager.supports_colors() {
            pager.spawn(format!("{}: {query}", "seal help".bold()), help.ansi())?;
        } else {
            pager.spawn(format!("seal help: {query}"), help)?;
        }
    } else {
        if want_color {
...
/// Find the command corresponding to a set of arguments, e.g., `["seal", "self"]`.
///
