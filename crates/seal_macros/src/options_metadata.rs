    ...
    match data {
        Data::Struct(DataStruct {
            ...
            for field in &fields.named {
                ...
                {
                    output.push(handle_field(field, attr)?);
                } else if field
                    .attrs
                    .iter()
            ...
            Ok(quote! {
                ...
                impl seal_options_metadata::OptionsMetadata for #ident {
                    fn record(visit: &mut dyn seal_options_metadata::Visit) {
                        #(#output);*
                    }
    ...
    match &field.ty {
        Type::Path(TypePath {
            ...
            Some(PathSegment {
                ...
                Ok(quote_spanned!(
                    ident.span() => (visit.record_set(#kebab_name, seal_options_metadata::OptionSet::of::<#path>()))
                ))
            }
            _ => Err(syn::Error::new(
                ident.span(),
...
/// Parse an `#[field(doc="...", default="...", value_type="...",
/// example="...")]` attribute and return data in the form of an `Field`.
fn handle_field(field: &Field, attr: &Attribute) -> syn::Result<TokenStream> {
        ...
        .find(|attr| attr.path().is_ident("deprecated"))
    {
        ...
        quote!(Some(seal_options_metadata::Deprecated { since: #since, message: #note }))
    } else {
        quote!(None)
    };
        ...
        quote!(
            Some(
                <#inner_type as clap::ValueEnum>::value_variants()
                    ...
                    .map(|value| seal_options_metadata::PossibleValue {
    ...
    Ok(quote_spanned!(
        ident.span() => {
            visit.record_field(#kebab_name, seal_options_metadata::Field{
                doc: &#doc,
                default: #default,
...
fn parse_field_attributes(attribute: &Attribute) -> syn::Result<FieldAttributes> {
    ...
    attribute.parse_nested_meta(|meta| {
        if meta.path.is_ident("default") {
            default = Some(get_string_literal(&meta, "default", "field")?.value());
        } else if meta.path.is_ident("value_type") {
            value_type = Some(get_string_literal(&meta, "value_type", "field")?.value());
        } else if meta.path.is_ident("scope") {
            scope = Some(get_string_literal(&meta, "scope", "field")?.value());
        } else if meta.path.is_ident("example") {
            let example_text = get_string_literal(&meta, "value_type", "field")?.value();
            example = Some(dedent(&example_text).trim_matches('\n').to_string());
        } else if meta.path.is_ident("possible_values") {
            possible_values = get_bool_literal(&meta, "possible_values", "field")?;
        } else {
            return Err(syn::Error::new(
                meta.path.span(),
                format!(
                    "Deprecated meta {:?} is not supported by seal's field macro.",
                    meta.path.get_ident()
                ),
            ));
        }
    ...
    let Some(value_type) = value_type else {
        return Err(syn::Error::new(
            attribute.span(),
            "Mandatory `value_type` field is missing in `#[field]` attribute. Specify the value type using `#[field(value_type=\"..\")]`.",
        ));
    };
    ...
    let Some(example) = example else {
        return Err(syn::Error::new(
            attribute.span(),
            "Mandatory `example` field is missing in `#[field]` attribute. Add an example using `#[field(example=\"..\")]`.",
        ));
    };
